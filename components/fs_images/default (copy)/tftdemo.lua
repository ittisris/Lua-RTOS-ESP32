...
tftdemo = {
	dispType = gdisplay.ILI9341, -- gdisplay.ILI9341 or gdisplay.ST7735B, gdisplay.ST7735, gdisplay.ST7735G
	maxx = 240,
	maxy = 320,
	miny = 12,
	touch = false,
...
function tftdemo.init()
	if gdisplay.gettype() < 0 then
		gdisplay.init(tftdemo.dispType, gdisplay.LANDSCAPE)
		if gdisplay.gettype() < 0 then
...
function tftdemo.thfullDemo()
	local orientation = gdisplay.LANDSCAPE
	local sec = 5
	while true do
		gdisplay.setrot(0);
		gdisplay.setcolor(gdisplay.CYAN)
